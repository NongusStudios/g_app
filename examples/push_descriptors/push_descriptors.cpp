    ...
    auto app = AppInit()
            ...
            .configure_vulkan_renderer([&](g_app::VulkanRendererInit& init){
                init.set_app_name("Cube")
                        ...
                        .set_enabled_device_extensions({VK_KHR_PUSH_DESCRIPTOR_EXTENSION_NAME})
                        .load_vkpfn({"vkCmdPushDescriptorSetKHR"});
            })
            .init();
    ...
    app.main_loop([&](const std::vector<Event>& events, const Time& time){
        ...
        *data = transform;
        ...
        auto writer = DescriptorWriter()
            .write_buffer(DescriptorSet(), 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, uniform_buffers[app.renderer().current_frame()]);
        ...
        cmd[app.renderer().current_frame()]
                ...
                .ext_push_descriptor_set(pipeline, VK_PIPELINE_BIND_POINT_GRAPHICS, 0, writer.get_writes())
